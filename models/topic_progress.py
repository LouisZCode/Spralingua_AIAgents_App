...
class TopicProgress(db.Model):
    ...
    # Unique constraint for user progress + topic number
    __table_args__ = (
        db.UniqueConstraint('user_progress_id', 'topic_number', name='_user_topic_uc'),
    )
    
    def __init__(self, user_progress_id, topic_number, total_exercises):
    ...
    def __repr__(self):
        status = "✓" if self.completed else f"{self.exercises_completed}/{self.total_exercises}"
        return f'<TopicProgress Topic{self.topic_number} {status}>'